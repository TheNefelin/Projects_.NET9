...
{
    private readonly IRepositoryByUser<UserAdventure> _repository;
    private readonly IUserGoogleRepository _userGoogleRepository;

    public UserAdventureService(IRepositoryByUser<UserAdventure> repository, IUserGoogleRepository userGoogleRepository)
    {
        _repository = repository;
        _userGoogleRepository = userGoogleRepository;
    }

    public async Task<ApiResponse<IEnumerable<UserAdventure>>> GetAllByUserIdAsync(Guid Id_User, CancellationToken cancellationToken)
    {
        try
        {
            var data = await _repository.GetAllByUserIdAsync(Id_User, cancellationToken);
            return new ApiResponse<IEnumerable<UserAdventure>>
            {
                IsSuccess = true,
                StatusCode = 200,
                Message = "Ok",
    ...
    public async Task<ApiResponse<object>> UpdateAsync(UserAdventureRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var userAdventure = new UserAdventure()
            {
                User_Id = request.UserToken.User_Id,
            ...
            var sqlResponse = await _repository.UpdateAsync(userAdventure, cancellationToken);
            return new ApiResponse<object>
            {
